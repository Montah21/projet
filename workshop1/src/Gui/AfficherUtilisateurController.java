...
public class AfficherUtilisateurController implements Initializable {
    ...
    int SelectedID = Listusers.getSelectionModel().getSelectedIndex();
    Listusers.getItems().remove(SelectedID);
  /*          List<utilisateur> list;
            
        utilisateur clicked=new utilisateur();
  
     clicked = Listusers.getSelectionModel().getSelectedItem();
    
    us.supprimerUtilisateur();
                
                list = FXCollections.observableList(as.getAll());
                Listusers.setItems((ObservableList<>) list);*/
